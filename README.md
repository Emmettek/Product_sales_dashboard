# Raport sprzedaży produktów
Ten projekt zawiera analizę sprzedaży produktów wraz z wizualizacjami i raportem. Celem jest przedstawienie wyników sprzedaży oraz umożliwienie dalszej analizy danych.
## Spis treści
- [Przegląd projektu](#Przegląd-projektu)
  - [Dane](#Dane)
  - [Wizualizacja](#Wizualizacja)
- [Proces Pracy](#Proces-Pracy)
  - [Technologie](#Technologie)
  - [Zdobyte Umiejętności](#Zdobyte-umiejętności)
 - [Autor](#Autor)
## Przegląd projektu
Celem projektu jest analiza sprzedaży produktów i przedstawienie wyników w formie raportu oraz wizualizacji przygotowanych w Power BI. Raport umożliwia interaktywne przeglądanie danych i analizę trendów sprzedaży.
### Dane
Dane możesz pobrać bezpośrednio z mojego repozytorium GitHub:
- [`InvoiceData.xlsx`](https://github.com/Emmettek/Product_sales_dashboard/blob/16fc5a8e912b27161bcd6d3a571a3d52627852f4/database/InvoiceData.xlsx)
- [`CustomerMaster.xlsx`](https://github.com/Emmettek/Product_sales_dashboard/blob/16fc5a8e912b27161bcd6d3a571a3d52627852f4/database/CustomerMaster.xlsx)
### Wizualizacja 
![Wizualizacja](sprzedaż_produktów_dashboard.png)
## Proces Pracy
### Technologie
- Power BI
### Zdobyte umiejętności
 - Ładowania i przygotowania danych w Power BI (Power Query)
 - Tworzenia relacji między tabelami i modeli danych
 - Projektowania wizualizacji oraz formatowania raportu
 - Tworzenia przejrzystego dashboardu do analizy sprzedaży
## Autor
- LinkedIn - [@gryczkaleksandra01](https://www.linkedin.com/in/gryczkaleksandra01/)
